...
<head>
  ...
  <title>Admin Dashboard - Event Messages</title>
  <!-- Bootstrap CSS -->
              ...
              <?php foreach ($accommodations as $index => $event): ?>
                <tr style="<?= ($event['is_seen'] == 0) ? 'font-weight: bold;' : ''; ?>">
                  ...
                  <td>
                    <div class="d-flex">
                      <!-- View Button -->
                      <a href="javascript:void(0);" class="btn btn-sm viewEvent" data-id="<?= esc($event['id']) ?>"
                        data-first-name="<?= esc($event['first_name']) ?>" data-last-name="<?= esc($event['last_name']) ?>"
                        data-phone="<?= esc($event['phone']) ?>" data-email="<?= esc($event['email']) ?>"
                        data-venue="<?= esc($event['venue']) ?>" data-message="<?= esc($event['message']) ?>" title="View"
                        onclick="markAsSeen(<?= $event['id']; ?>)">
                        <img src="/src/eye.png" width="26" height="26" alt="">
                      </a>
                      <!-- Email Icon with mailto link -->
                      <a href="mailto:<?= urlencode(esc($event['email'])) ?>" class="btn btn-sm" title="Send Email">
                        <img src="/src/gmail.png" width="26" height="26" alt="Email">
                      </a>
                      <!-- Delete Button -->
                      <a href="<?= site_url('/Admin/delete_EventMessage/' . $event['id']) ?>" class="btn btn-sm"
